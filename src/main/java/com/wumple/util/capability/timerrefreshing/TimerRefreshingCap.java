...
abstract public class TimerRefreshingCap<T extends IThing, W extends Expiration, X extends IThing> extends TickingThingCap<T> implements ITimerRefreshingCap<T, W>
{
    ...
    */
    ...
    {
        LazyOptional< ? extends IEventTimedThingCap<X,W>> lcap = !stack.isEmpty() ? getTimedCap(stack) : null;

        return lcap.map(cap-> rescheduleAndCheck(cap, index, itemhandler, stack, time)).orElse(false);
    }
